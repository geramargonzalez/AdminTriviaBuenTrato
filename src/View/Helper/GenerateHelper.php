...
class GenerateHelper extends Helper
{
    ...
    public function image($name, $size = 12) {
        $form = $this->Html->image('avatar.jpg', ['id' => 'show-file', 'class' => 'img-responsive img-circle']) . $this->Form->file($name, ['id' => 'select-file']);
    ...
    public function bin($name, $select = false) {
        return '<div class="btn-group" data-toggle="buttons" style="width: 100%;">' . $this->option($name, 1, $select) . $this->option($name, 0, !$select) . '</div>';
    }

    private function option($name, $value, $select = false) {
        return '<label class="btn btn-default ' . ($select ? 'active' : '') . ' ' . ($value ? 'danger' : '') . '" style="width: 49%;"><input type="radio" name="' . $name . '" value="' . $value . '" checked="' . ($select ? 'checked' : '') . '">' . ($value ? __('Yes') : __('No')) . '</label>';
    }
}
